...
class EmployeeCRUDCBV(HttpResponseMixin,SerializeMixin,View):
    ...
    def put(self,request,*args,**kwargs):
        ...
        obj=self.get_object_by_id(id)
        if obj is None:
            json_data=json.dumps({'msg':'No match record found,Not possible to perform updation'})
    ...
    def delete(self,request,*args,**kwargs):
        ...
        obj=self.get_object_by_id(id)
        if obj is None:
            json_data=json.dumps({'msg':'No match record found,Not possible to perform delete operation'})
            return self.render_to_http_response(json_data,status=404)
